...
function renderFaultCodeList(){
  ...
  if(!listEl) return;
  const q=state.codeQ || '';
  let arr = q ? rankFaultCodes(q) : faultCodes.slice(0,60);
  if($('codeResultInfo')) $('codeResultInfo').textContent = q ? `匹配 ${arr.length} 条故障代码` : `已导入 ${faultCodes.length} 条故障代码，默认显示前 60 条`;
  if(!arr.length){ listEl.innerHTML='<div class="empty">没有匹配故障代码。可以输入 601、制动压力、CAN、电机温度等关键词。</div>'; return; }
  listEl.innerHTML = arr.slice(0,120).map(c=>`<article class="fault-code-card risk-${escapeHtml(c.risk)}"><div class="code-top"><b>${escapeHtml(c.code)}</b><span>${escapeHtml(c.levelName||'未标注')}</span></div><h3>${escapeHtml(c.name)}</h3><p><b>系统：</b>${escapeHtml(c.system)} ｜ <b>风险：</b>${escapeHtml(c.risk)}</p><p>${escapeHtml(c.advice)}</p><small>${escapeHtml(c.source||'')}</small></article>`).join('');
}
